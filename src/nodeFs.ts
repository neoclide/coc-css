...
import { RequestService, FileType, FileStat } from './requests'
...
export function getNodeFSRequestService(): RequestService {
  ...
  return {
    getContent(location: string, encoding?: BufferEncoding): Promise<string> {
    ...
    stat(location: string): Promise<FileStat> {
      ensureFileUri(location)
      return new Promise((c, e) => {
        const uri = Uri.parse(location)
    ...
    readDirectory(location: string): Promise<[string, FileType][]> {
      ensureFileUri(location)
      return new Promise((c, e) => {
